...
async function sendBookingNotifications(booking: {
  ...
  preferredDate: string; address: string; notes: string;
}, status: string) {
  const messages: Record<string, { subject: string; heading: string; body: string }> = {
    confirmed: {
      subject: 'Your GoClean Booking is Confirmed!',
      heading: '📅 Booking Confirmed',
      body: `Your <strong>${booking.service}</strong> booking on <strong>${booking.preferredDate}</strong> has been confirmed! Our team will be there as scheduled. If you need to reschedule, please call us at 0917 117 8605.`,
    },
    completed: {
  ...
  if (apiKey && booking.email) {
    const html = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f9f9f9;padding:24px;border-radius:8px;">
        ...
        <p style="font-size:13px;color:#555;">Questions? Call us at <strong>0917 117 8605</strong> or email <strong>gocleanair@gmail.com</strong></p>
    ...
    fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: 'GoClean Aircon <onboarding@resend.dev>', to: [booking.email], subject: template.subject, html }),

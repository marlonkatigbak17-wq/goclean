...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      {/* Sidebar */}
      <aside className="w-56 bg-[#0f1f5c] text-white flex flex-col shrink-0 h-screen sticky top-0">
        ...
        <nav className="flex-1 py-2 overflow-y-auto min-h-0">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-3 px-5 py-2.5 text-sm text-blue-100 hover:bg-white/10 hover:text-white transition-colors"
            >
              <item.icon size={16} />
              {item.label}
            </Link>
